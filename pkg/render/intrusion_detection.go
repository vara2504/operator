...
func (c *intrusionDetectionComponent) webhooksControllerContainer() corev1.Container {
	envVars := []corev1.EnvVar{
		{
			Name:  "LINSEED_URL",
			Value: relasticsearch.LinseedEndpoint(c.SupportedOSType(), c.cfg.ClusterDomain, ElasticsearchNamespace, c.cfg.ManagedCluster),
		},
		{
			Name:  "LINSEED_CA",
...
func (c *intrusionDetectionComponent) intrusionDetectionControllerContainer() corev1.Container {
	envs := []corev1.EnvVar{
		...
		{
			Name:  "LINSEED_URL",
			Value: relasticsearch.LinseedEndpoint(c.SupportedOSType(), c.cfg.ClusterDomain, LinseedNamespace(c.cfg.Tenant), c.cfg.ManagedCluster),
		},
		{
			Name:  "LINSEED_CA",
			Value: c.cfg.TrustedCertBundle.MountPath(),
