...
var _ = Describe("IntrusionDetection controller tests", func() {
	...
	Context("Feature intrusion detection not active", func() {
		...
		It("should not create resources", func() {
			mockStatus.On("SetDegraded", string(operatorv1.ResourceValidationError), "Feature is not active - License does not support this feature").Return()
			mockStatus.On("SetDegraded", string(operatorv1.ResourceNotFound), "Elasticsearch secrets are not available yet, waiting until they become available - Error: secrets \"tigera-ee-installer-elasticsearch-access\" not found").Return().Maybe()
