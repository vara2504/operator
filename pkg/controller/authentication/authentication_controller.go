...
func (r *ReconcileAuthentication) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if variant != oprv1.TigeraSecureEnterprise {
		...
		r.status.SetDegraded(oprv1.ResourceNotReady, fmt.Sprintf("Waiting for network to be %s", oprv1.TigeraSecureEnterprise))
		return reconcile.Result{}, nil
	}
	...
	if managementClusterConnection != nil {
		...
		r.status.SetDegraded(oprv1.ResourceValidationError, "Only one of Authentication and ManagementClusterConnection may be specified")
		return reconcile.Result{}, err
	} else if err != nil {
