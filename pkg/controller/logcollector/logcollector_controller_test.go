...
var _ = Describe("LogCollector controller tests", func() {
	...
	Context("image reconciliation", func() {
		BeforeEach(func() {
			...
			mockStatus.On("SetDegraded", operatorv1.ResourceNotReady, "Waiting for LicenseKeyAPI to be ready").Return().Maybe()
		...
		Context("Forward to S3", func() {
			...
			Context("Disable feature via license", func() {
				...
				It("should not forward logs to s3", func() {
					mockStatus.On("SetDegraded", operatorv1.ResourceValidationError, "Feature is not active - License does not support feature: export-logs").Return()
					_, err := r.Reconcile(ctx, reconcile.Request{})
					Expect(err).ShouldNot(HaveOccurred())
					...
					Status: operatorv1.TigeraStatusStatus{
						Conditions: []operatorv1.TigeraStatusCondition{
							...
							},
							{
								...
								Reason:  operatorv1.NotApplicable,
